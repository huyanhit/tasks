...
const formatMessage = function() {
    if(props.item){
        ...
        if (props.item.status !== 0) {
            ...
            content = processRoomAddMember(content);
...
const processRoomCreate = function(content) {
    return content.replace(Regex.ROOM.CREATE, matches => {
        let msg  = '<div class="room-create d-flex">' +
            '<span>{auth}</span> ' +
            '<span> Đã tạo room </span> ' +
            '<span>{room}</span></div>';
...
const processRoomAddMember = function(content) {
    return content.replace(Regex.ROOM.ADD_MEMBER, matches => {
        let msg = '<div class="room-add-member">' +
            '{auth} <span> Đã thêm các thành viên vào room</span> ' +
            '{members} </div>';
        let auth = processTagAuth(matches);
